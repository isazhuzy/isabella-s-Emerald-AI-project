...
def main() -> int:
    ...
    if args.push and not settings.has_loxo:
        print("✖  --push requested but Loxo isn't configured (see .env.example).", file=sys.stderr)
        return 2

    result = run_pipeline(transcript, client_name=args.client, push_to_loxo=args.push)
